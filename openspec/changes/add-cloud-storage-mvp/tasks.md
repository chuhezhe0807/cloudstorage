...
- [ ] 10.1 文件列表页：面包屑、列表/网格切换、react-virtual 虚拟滚动
- [ ] 10.2 右键菜单与操作弹窗：重命名、移动（目录选择树）、删除、分享、下载
- [ ] 10.3 自研上传组件：拖拽 + 选择、crypto.subtle 算 SHA-256 调秒传、分片并发上传、断点续传、进度条、失败重试
- [ ] 10.4 下载：处理 302 预签名重定向
- [ ] 10.5 回收站页：列表、还原、永久删除
- [ ] 10.6 搜索页：名称 + 过滤条件 + 分页
- [ ] 10.7 分享创建弹窗 + 分享管理页 + 分享访问页（提取码输入）
- [ ] 10.8 通知列表页 + 标记已读
