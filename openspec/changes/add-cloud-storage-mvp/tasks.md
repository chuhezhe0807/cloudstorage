...
- [x] 11.1 JaCoCo 插件配置，覆盖率门槛 70%，`mvn verify` 失败即阻断
- [x] 11.2 全部后端接口 RestAssured 用例补齐，关键路径 E2E（Playwright）
- [x] 11.3 Playwright E2E：登录/上传/下载/分享/回收站/语言切换/主题切换
- [x] 11.4 SkyWalking 全链路验证：上传链路 trace 完整可见
- [x] 11.5 安全测试用例集：路径穿越 / 跨租户访问 / 提取码爆破 / 预签名 URL 过期

## 12. 可水平扩展与打磨

- [x] 12.1 各服务多实例本地启动验证（不同端口）
- [x] 12.2 Nginx upstream 配置 + 前端静态资源托管
- [x] 12.3 Sentinel 限流熔断规则调优
- [x] 12.4 JMeter 压测：分片上传并发、目录列表 QPS、下载带宽
- [x] 12.5 文档：`README.md` 本地启动步骤、架构图、API 概览
- [x] 12.6 JaCoCo 覆盖率达标复核
