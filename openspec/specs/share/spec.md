...
### Requirement: 查看分享信息
访客 SHALL 可通过分享 code 查看分享基本信息（文件名、大小、是否目录、过期时间、剩余下载次数），无需提取码。系统 MUST 校验分享存在且有效（未过期、未耗尽），否则返回对应错误。

#### Scenario: 查看有效分享信息
- **WHEN** 访客请求有效分享 code 的信息
- **THEN** 返回文件名、大小、是否目录、过期时间、最大下载次数、已下载次数，不要求提取码

#### Scenario: 查看已过期分享信息
- **WHEN** 访客请求已过期的分享信息
- **THEN** 返回 410 Gone（错误码 SHARE_EXPIRED）

#### Scenario: 查看不存在的分享
- **WHEN** 访客请求不存在的分享 code 信息
- **THEN** 返回 404 Not Found（错误码 SHARE_NOT_FOUND）
