...
/**
 * 全局 JWT 鉴权过滤器。
 * 白名单路径放行；其他请求校验 Bearer Token，将 tenantId/userId/locale 注入请求头。
 */
@Slf4j
@Component
...
public class JwtAuthFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        try {
            ...
            if (locale != null && !locale.isBlank()) {
                request = request.mutate().header("X-Locale", locale).build();
            }
    ...
    private boolean isWhitelisted(String path) {
        ...
        for (String prefix : SHARE_ACCESS_PREFIX) {
            if (path.startsWith(prefix) && (path.endsWith("/access") || path.endsWith("/info") || path.endsWith("/download"))) {
                return true;
            }
        }
        return false;
